...
class MoyKlassCRM:
    ...
    async def get_user_phone_by_id(self, user_id: int) -> Optional[str]:
        """Получить телефон ученика по его ID в MoyKlass."""
        headers = await self._get_headers()
        if not headers:
            return None
        try:
            async with httpx.AsyncClient() as client:
                # Сначала пробуем прямой эндпоинт GET /users/{id}
                resp = await client.get(
                    f"{MOYKLASS_BASE_URL}/users/{user_id}", headers=headers
                )
                if resp.status_code == 200:
                    user = resp.json()
                    phone = user.get("phone")
                    if phone:
                        logger.info(f"get_user_phone_by_id: id={user_id} → {phone}")
                        return phone
                ...
                if resp2.status_code == 200:
                    ...
                    if users:
                        ...
                        if phone:
                            logger.info(f"get_user_phone_by_id (fallback): id={user_id} → {phone}")
                            return phone
        except Exception as e:
            logger.error(f"get_user_phone_by_id error: {e}")
        logger.warning(f"get_user_phone_by_id: телефон не найден для id={user_id}")
        return None

    async def get_phones_for_lesson(self, lesson_id: int) -> List[str]:
        """Телефоны учеников, записанных на конкретное занятие."""
        headers = await self._get_headers()
        if not headers:
            return []
        phones: List[str] = []
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{MOYKLASS_BASE_URL}/lessonRecords", headers=headers,
                ...
                for rec in records:
                    ...
                    if not uid:
                        continue
                    phone = await self.get_user_phone_by_id(uid)
                    if phone:
                        phones.append(phone)
        except Exception as e:
            logger.error(f"get_phones_for_lesson error: {e}")
        return phones

    async def get_phones_for_class(self, class_id: int) -> List[str]:
        """Телефоны учеников с активной записью в группу (statusId=2)."""
        headers = await self._get_headers()
        if not headers:
            return []
        phones: List[str] = []
        try:
            async with httpx.AsyncClient() as client:
                resp = await client.get(
                    f"{MOYKLASS_BASE_URL}/joins", headers=headers,
                    params={"classId": class_id, "statusId": 2, "limit": 200}
                )
                if resp.status_code != 200:
                    return []
                joins = resp.json().get("joins", []) if isinstance(resp.json(), dict) else resp.json()
                for j in joins:
                    uid = j.get("userId")
                    if not uid:
                        continue
            ...
            logger.error(f"get_phones_for_class error: {e}")
        return phones
...
# Шаблоны WhatsApp-сообщений по коду события MoyKlass.
# None = событие распознаётся, но не отправляется клиенту (пропускаем без ошибки).
_CRM_EVENT_TEMPLATES: Dict[str, Optional[str]] = {
    # Платежи и абонементы
    "payment_new": (
        "Здравствуйте. Оплата получена — спасибо. Если есть вопросы, пишите."
    ),
    "sub_end_days": (
        "Здравствуйте. Напоминаем: абонемент заканчивается {endDate}. "
        "Чтобы продолжить занятия без перерыва, обратитесь к управляющему вашего филиала."
    ),
    "sub_days_next_payment": (
        "Здравствуйте. Напоминаем о предстоящем платеже по абонементу. "
        "Уточните детали у управляющего вашего филиала."
    ),
    "sub_lesson_in_debt": (
        "Здравствуйте. Занятие проведено, но баланс недостаточен — оно засчитано в долг. "
        "Пожалуйста, пополните баланс — обратитесь к управляющему вашего филиала."
    ),
    "sub_lessons_left": (
        "Здравствуйте. В абонементе осталось мало занятий. "
        "Чтобы не прерывать обучение, свяжитесь с управляющим вашего филиала для продления."
    ),
    # Занятия
    "lesson_start_hours": (
        "Здравствуйте. Напоминаем: сегодня занятие в {beginTime}. Ждём вас."
    ),
    "lesson_start_days": (
        "Здравствуйте. Напоминаем: занятие {date}. До встречи."
    ),
    "lesson_mark_set": (
        "Здравствуйте. Преподаватель выставил оценку за {type_text}: {value}."
    ),
    # Группы
    "class_start_hours": (
        "Здравствуйте. Напоминаем: сегодня старт вашей группы. Ждём вас."
    ),
    "class_start_days": (
        "Здравствуйте. Напоминаем: ваша группа стартует {beginDate}. "
        "Если есть вопросы, напишите нам."
    ),
    # Записи
    "join_new": (
        "Здравствуйте. Ваша заявка на обучение получена. "
        "Управляющий свяжется с вами в ближайшее время."
    ),
    "join_changed_state": None,   # обрабатывается отдельно ниже
    # Посещаемость
    "user_consecutive_visit_missed_2": (
        "Здравствуйте. Заметили, что занятия пропускаются. "
        "Если возникли трудности — напишите, мы готовы помочь."
    ),
    # Преподаватели и задания — не отправляем клиенту
    "lesson_changed": None,
    "lesson_record_new": None,
    "lesson_record_changed": None,
    "lesson_task_new": None,
    "lesson_task_changed": None,
    "task_new": None,
    "task_changed": None,
    "task_deleted": None,
}

# Сообщения при смене статуса записи (join_changed_state)
_JOIN_STATUS_TEXTS: Dict[int, str] = {
    2: "Ваша запись в группу подтверждена. Занятия начнутся согласно расписанию.",
    3: "Ваша запись завершена. Если возникнут вопросы — пишите.",
    4: "Ваша запись отменена. Если хотите записаться снова — напишите нам.",
}


def _build_crm_message(event: str, obj: dict) -> Optional[str]:
    """Формирует текст WhatsApp-сообщения по событию CRM."""
    # join_changed_state — зависит от statusId
    if event == "join_changed_state":
        return _JOIN_STATUS_TEXTS.get(obj.get("statusId"))

    template = _CRM_EVENT_TEMPLATES.get(event, "UNKNOWN")
    if template == "UNKNOWN":
        return None  # неизвестное событие — пропускаем
    if template is None:
        return None  # известное, но не для клиента

    try:
        if event == "sub_end_days":
            return template.format(endDate=obj.get("endDate", ""))
        if event in ("lesson_start_hours",):
            return template.format(beginTime=obj.get("beginTime", ""))
        if event == "lesson_start_days":
            return template.format(date=obj.get("date", ""))
        if event == "class_start_days":
            return template.format(beginDate=obj.get("beginDate", ""))
        if event == "lesson_mark_set":
            type_map = {"home": "домашнее задание", "lesson": "задание на уроке"}
            type_text = type_map.get(obj.get("type", ""), "задание")
            return template.format(type_text=type_text, value=obj.get("value", ""))
        return template  # шаблон без переменных
    except (KeyError, ValueError):
        return template  # fallback без форматирования


def _phone_to_chat_id(phone: str) -> str:
    """Приводит телефон к формату Green API chat_id."""
    clean = re.sub(r"[^\d]", "", phone)
    if len(clean) == 10:
...
@app.post("/moyklass-webhook")
async def handle_moyklass_webhook(request: Request):
    """Принимает вебхуки из сценариев MoyKlass и отправляет WhatsApp-сообщения."""
    try:
        data = await request.json()
    except Exception as e:
        logger.warning(f"MoyKlass webhook: невалидный JSON: {e}")
        return {"status": "ok"}

    event = data.get("event", "")
    obj: dict = data.get("object") or {}
    logger.info(f"MoyKlass webhook event={event!r} obj={json.dumps(obj)[:300]}")

    message = _build_crm_message(event, obj)
    if message is None:
        logger.info(f"MoyKlass webhook event={event!r}: нет шаблона, пропускаем")
        return {"status": "ok"}

    # --- Определяем список получателей ---
    phones: List[str] = []

    # Прямой телефон в объекте (user_* события)
    if obj.get("phone"):
        phones.append(obj["phone"])

    # Телефон через userId
    elif obj.get("userId"):
        phone = await crm.get_user_phone_by_id(obj["userId"])
        if phone:
            phones.append(phone)

    # Для событий занятия без userId — берём всех записанных учеников
    elif obj.get("lessonId"):
        phones = await crm.get_phones_for_lesson(obj["lessonId"])

    # Для событий группы без userId — берём всех учеников группы
    elif obj.get("classId"):
        phones = await crm.get_phones_for_class(obj["classId"])

    if not phones:
        logger.warning(f"MoyKlass webhook event={event!r}: не удалось определить получателей")
        return {"status": "ok"}

    # --- Отправляем WhatsApp-сообщения ---
    for phone in phones:
        chat_id = _phone_to_chat_id(phone)
        logger.info(f"MoyKlass → WA {chat_id}: {message[:100]}")
        try:
